...
*   **Cross-Platform Support:** Works on macOS, Linux, and Windows (via Git Bash, WSL, or PowerShell).
...
*   **`src/statusline.ps1`**: A PowerShell script that provides the same functionality as `statusline.sh` for Windows users. It reimplements the necessary logic in PowerShell, as it cannot directly use the Bash functions.
*   **`src/git-hooks/prepare-commit-msg`**: A Git hook that automatically appends the current Jira ticket to commit messages.
*   **`install.sh` and `uninstall.sh`**: Scripts for installing and uninstalling the tracker. The installer copies files to the appropriate locations, sets up Git hooks, and modifies the user's shell configuration. The uninstaller reverses these actions.
*   **`~/.claude_code_work/`**: The directory where work session data is stored in JSON files, one for each ticket.
*   **`~/.claude_code_config`**: A configuration file that stores the default Jira ticket prefix.
...
*   The status line is updated by Claude Code, which executes the `statusline.sh` or `statusline.ps1` script at regular intervals.
...
*   `set_ticket_prefix ABC`: Sets the default ticket prefix.
